...
def load_uav_records(txt_path: str):
    """Parse your OpuInertialJointData file into a list of dicts.

    The file is formatted like:
        ["k": v, ...]["k": v, ...]...
    so we split on '][' and wrap each chunk with { }.
    """
    ...
    for i, p in enumerate(parts):
        ...
        try:
            ...
            # If there's a rare malformed chunk, skip it rather than crashing
            continue
    return records
...
def yawrate_from_deg(yaw_prev_deg: float, yaw_curr_deg: float, dt: float) -> float:
    """Compute yaw-rate (rad/s) from two yaw angles in degrees with wrap handling."""
...
DT = 0.1  # time tick [s]
SIM_TIME = 300.0  # simulation time [s]
MAX_RANGE = 20.0  # maximum observation range
M_DIST_TH = 2.0  # Threshold of Mahalanobis distance for data association.
STATE_SIZE = 3  # State size [x,y,yaw]
LM_SIZE = 2  # LM state size [x,y]

show_animation = False  # set True to animate
...
def main():
    ...
    # === INPUT DATA ===
    # Each parsed record is one timestep. We use vgx/vgy (velocity) and yaw to build control inputs.
    # If you have a different file, change this path.
    uav_txt_path = "/Users/kordelfrance/Documents/School/UTD/PhD Thesis/Ghosts/ChasingGhosts/robot/utils/inertial/OpuInertialJointData 6.txt.txt"
    records = load_uav_records(uav_txt_path)

    if len(records) < 2:
        raise RuntimeError(f"Not enough records parsed from {uav_txt_path}")

    # Optional: downsample for faster plotting/iteration
    # step = 1 uses every record; step = 2 uses every other, etc.
    step = 1
    records = records[::step]

    # RFID positions [x, y] (kept from original example; used to generate synthetic range/bearing measurements)
    ...
    # Initialize yaw from first record if present
    if "yaw" in records[0]:
        xTrue[2, 0] = math.radians(records[0]["yaw"])
        xDR[2, 0] = xTrue[2, 0]
        xEst[2, 0] = xTrue[2, 0]
    ...
    # Iterate over file records as timesteps
    for k in range(1, len(records)):
        # Build control input u = [v; yawrate]
        vgx = float(records[k].get("vgx", 0.0))
        vgy = float(records[k].get("vgy", 0.0))

        # Speed magnitude; if vgx/vgy are body-frame velocities, this approximates forward speed.
        v = math.hypot(vgx, vgy)

        # yaw-rate from successive yaw measurements, if yaw present; else 0
        ...
        # Use the original observation model to produce:
        # - xTrue: "true" integrated pose
        # - xDR: dead reckoning with noisy control
        # - z: range/bearing to beacons within MAX_RANGE
    ...
    # Final plot (non-animated)
    plt.figure()
    plt.plot(RFID[:, 0], RFID[:, 1], "*k", label="RFID beacons")
    ...
    plt.grid(True)
    plt.legend()
    plt.title("UAV path from TXT-derived control inputs")
    plt.show()
