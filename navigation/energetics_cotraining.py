...
class DQNAgent:
    def __init__(self, state_dim, action_dim, memory, gamma=0.99, lr=1e-3, trace_decay=0.9):
        self.gamma = gamma
    ...
    def td_error(self, state, action, reward, next_state, done):
        with torch.no_grad():
            ...
            q_value = self.policy_net(state)[0, action]
            target_q = reward
            if not done:
                target_q += self.gamma * self.target_net(next_state).max(1)[0].item()
            return abs(q_value.item() - target_q)



def evaluate(agent, env, episodes=100):
    print("starting evaluation")    # Evaluate agent1
    total_rewards = []
    ...
    print(f"Agent 1 Avg Reward: {avg_reward:.2f}")
...
if __name__ == '__main__':
    ...
    num_episodes = 5
    self_paced_threshold = 1
    batch_size = 32
    rewards = []

    agent1 = DQNAgent(state_dim=57, action_dim=6, memory=ReplayMemory(10000))
    agent2 = DQNAgent(state_dim=57, action_dim=6, memory=ReplayMemory(10000))

    # Unlabeled buffer for state-action-reward pseudo-label exchange
    unlabeled_pool = []
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            # Choose randomly who acts this step
            acting_agent = random.choice([1, 2])
            action = a1 if acting_agent == 1 else a2
            next_state, reward, done, _ = env.step(action)
            total_reward += reward

            # Store state-action in unlabeled buffer
            unlabeled_pool.append((state, action, next_state, done))

            state = next_state

        rewards.append(total_reward)

        # ---------- Self-Paced Pseudo-Reward Estimation ----------
        confident_samples = []
        for s, a, s_next, d in unlabeled_pool:
            r1 = agent1.policy_net(torch.FloatTensor(s))[a].item()
            r2 = agent2.policy_net(torch.FloatTensor(s))[a].item()
            pseudo_reward = (r1 + r2) / 2

            td1 = agent1.td_error(s, a, pseudo_reward, s_next, d)
            td2 = agent2.td_error(s, a, pseudo_reward, s_next, d)
            avg_td = (td1 + td2) / 2

            if avg_td < self_paced_threshold:  # SPL filter
                confident_samples.append((s, a, s_next, pseudo_reward, d))

        # ---------- Agents train on confident pseudo-labeled samples ----------
        for sample in confident_samples:
            agent1.memory.push(*sample)
            agent2.memory.push(*sample)
        ...
        # Slowly increase self-paced threshold to allow harder samples later
        self_paced_threshold *= 1.05
